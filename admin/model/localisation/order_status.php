...
class ModelLocalisationOrderStatus extends Model {
	...
	/*
	public function getTicketStatus($order_status_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product_status WHERE order_status_id = '" . (int)$order_status_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	*/
	public function getTicketStatuses($data = array()) {
      	if ($data) {
			...
			$order_status_data = $this->cache->get('order_product_status.' . (int)$this->config->get('config_language_id'));
		
			if (!$order_status_data) {
				$query = $this->db->query("SELECT order_status_id, name FROM " . DB_PREFIX . "order_product_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
	
				$order_status_data = $query->rows;
			
				$this->cache->set('order_product_status.' . (int)$this->config->get('config_language_id'), $order_status_data);
			}	
