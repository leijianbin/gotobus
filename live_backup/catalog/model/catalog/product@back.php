...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id,$ticket_date) {
		...
		$sql = 	"SELECT DISTINCT pd.* , p.*, p2c.* 
				FROM product p, product_description pd, product_to_category p2c
				WHERE p.product_id =" . (int)$product_id ." AND p.product_id = pd.product_id
		...
		if ($query->num_rows) {
				if(in_array(date("w",strtotime($ticket_date)),explode("|",$query->row['special_date']))){
					$is_special_date=1;
				}else{
					$is_special_date=0;
				}
			return array(
				...
				'category_arrival_id'	   => $query->row['category_arrival_id'],
				'departure_time'	=> $query->row['departure_time'],
				'arrive_time'	    => $query->row['arrive_time'],
				'departure_station' => $query->row['departure_station'],
				'arrive_station'    => $query->row['arrive_station']
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
				$sql .= " WHERE ( p2c.category_id = " . $data['filter_category_id']. " AND p.product_id = p2c.product_id" ; 
			}
