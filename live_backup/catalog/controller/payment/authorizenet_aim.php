	...
	public function send() {
		...
		if (curl_error($curl)) {
			...
			if ($response_info[1] == '1') {
				...
				$json['success'] = $this->url->link('checkout/success', '', 'SSL') .'&order_id='. $this->session->data['order_id']. '&confirm_no='. $order_info['invoice_no'];
			} else {
				$json['error'] = $response_info[4];
				//$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('config_order_status_id'));
				//$json['error'] = $this->url->link('checkout/success', '', 'SSL') .'&order_id='. $this->session->data['order_id']. '&confirm_no='. $order_info['invoice_no'];
