...
class ControllerCheckoutCart extends Controller {
	...
	public function addBus() {
		...
		if (isset($this->request->get['product_return_id'])) {
			$product_return_id = $this->request->get['product_return_id'];
		} else {
			$product_return_id = 0;
		}

		if (isset($this->request->get['filter_date_return'])) {
			$filter_date_return = $this->request->get['filter_date_return'];
		} else {
		...
		if ($product_return_info) {
			...
			$product_return_options = $this->model_catalog_product->getProductOptions($this->request->get['product_return_id']);
			
			foreach ($product_return_options as $product_return_option) {
				if ($product_return_option['required'] && empty($option[$product_return_option['product_option_id']])) {
					$json['error']['option'][$product_return_option['product_option_id']] = sprintf($this->language->get('error_required'), $product_option['name']);
				}
			}
