...
class ControllerProductSearch extends Controller { 	
	public function index() { 
		...
		if (isset($this->request->get['passenger_no'])) { //search keyword as bus_type
		...
		if (isset($this->request->get['category_arrival_id'])) {
			$category_arrival_id = $this->request->get['category_arrival_id'];
		} else {
			$category_arrival_id = 0;
		} 

		// add filter_date and filter_date_return
		// 
		...
		if (isset($this->request->get['filter_return_date'])) {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['filter_tag'])) {
			$data = array(
				...
				'filter_category_arrival_id'  => $category_arrival_id,
				'filter_date'  => $filter_date, 
				'filter_return_date'  => $filter_return_date,
			...
			foreach ($results as $result) {
				...
				*/
				if($result['category_id'] == $data['filter_category_id'] )
				{
					$bus_type = "single";
					//$bus_date = $filter_date;
				}
				else
				{
					$bus_type = "return";
					//$bus_date = $filter_date_return;
				}
				...
				$this->data['products'][] = array(
					...
					'arrive_station' => $result['arrive_station'],
					'category_id' => $result['category_id'],
					'category_arrival_id' => $result['category_arrival_id'],
					'model' => $result['model'],
					'quantity' => $result['quantity'],
					'passenger_no'=> $passenger_no,
					'bus_type' => $bus_type,
					'bus_date' => $filter_date,
					'bus_date_return' => $filter_return_date,
					//'special'     => $special,
					//'tax'         => $tax,
			...
			if (isset($this->request->get['category_arrival_id'])) {
				$url .= '&category_arrival_id=' . $this->request->get['category_arrival_id'];
			}
